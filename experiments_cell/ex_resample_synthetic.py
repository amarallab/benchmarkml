...
def scripting(i):
    pyTitle = f'RESAMPLED48_SYNTHETIC_E{i}'
...
def get_data(n_noisy = 100, shuffle=False):
    import random
    FileName = '../cell_data/synthtic.csv'
    use_columns = [1,2] + [i+3 for i in range(n_noisy)] + [8408]
    columns = ['signal1', 'signal2'] + ['noisy'+str(i+2) for i in range(n_noisy)]  +  ['label']
...
def main(i):
    n_noisy_list = [0,2,10,50,100,200,400,800,1000,2000,5000,8000]
    repeats = 20
